...
EOT;
    ...
    /**
     * calculate the 16 byte version of the 128 bit SHA1 digest of the string
     *
     * @param $string
     * @return string
     */
    function SHA1_16($string)
    {
        $tmp = SHA1($string);
        $out = '';
        for ($i = 0; $i <= 30; $i = $i + 2) {
    ...
    function output($debug = false)
    {
        ...
        if ($this->fileIdentifier === '') {
            ...
            $this->fileIdentifier = SHA1('DOMPDF' . __FILE__ . $tmp . microtime() . mt_rand());
        }
