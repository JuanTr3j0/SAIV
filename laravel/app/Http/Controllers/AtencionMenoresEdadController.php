...
class AtencionMenoresEdadController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $select = [
                DB::raw('md5(atencion_menores_edads.id) as "key"'),
    ...
    public function store(Request $request)
    {
        try{
            ...
            if($request -> correlativo !== null){
                $correlativoExist = AtencionMenoresEdad::whereRaw('md5(id) not like "'.$request->key.'"')
                ...
                if($correlativoExist)
                    ...
                    AtencionMenoresEdad::whereRaw('md5(id) like "'.$request->key.'"')->where('estado', true)->update(['correlativo' => intval($request->correlativo)]);
            }
            // Creo o actualizo un registro
            $atencionMenores = AtencionMenoresEdad::whereRaw('md5(id)  like "'.$request->key.'"')->first();
            ...
            if(boolval($request->casoSaiv) && ($request->codigoCaso ?? null) !== null)
                    $atencionMenores->caso_fk = Casos::where('estado', true)->whereRaw('md5(id) = "'.((object) $request -> codigoCaso) -> key.'"')->first()->id;
                else 
            ...
            return response()->json([
                "key"           => DB::select(DB::raw("select md5(".$atencionMenores->id.") as 'key';"))[0]->key,
    ...
    {  
        try {
            ...
            $atencion = AtencionMenoresEdad::select($select)->whereRaw('md5(id) like "'.$key.'"')->first();
    ...
    {
        try{
           return AtencionMenoresEdad::whereRaw('md5(id) like "'. ($request->key ?? null).'"')->update(['estado' => false]);
        } catch (\Exception $e) {
            bitacora_errores('AtencionMenoresEdadController.php', $e);
            return response()->json(['error' => 'Linea -> '.$e->getLine().' Error -> '.$e->getMessage()]);
        }
