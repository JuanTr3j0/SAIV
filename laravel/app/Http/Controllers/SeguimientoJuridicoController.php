...

{
    ...
    public function index(Request $request){
        ...
        try {
            ...
            $select = [
                ...
                DB::raw('SHA1(seguimiento_juridicos.id) as "key"'),
...

    {
        ...
        try{
            ...
            $seguimiento_juridico = SeguimientoJuridico::whereRaw('SHA1(id)  = "'.$request->key.'"')->first();
            ...
            return response()->json([
                ...
                "key" => DB::select(DB::raw("select SHA1(".$seguimiento_juridico->id.") as 'key';"))[0]->key,
    ...
    private function keyAtencionBrindada($request) {
        ...
        if($request->atencionBrindadaSubMenu != null)
            ...
            return AtencionBrindada::whereRaw('SHA1(id)  = "'.$request->atencionBrindadaSubMenu.'"')->first()->id;

        

        return AtencionBrindada::whereRaw('SHA1(id)  = "'.$request->atencionBrindada.'"')->first()->id;  
...

    {
        ...
        try{
            ...
            if(!SeguimientoJuridico::whereRaw('SHA1(id)  = "'.$key.'"')->exists()){
            ...
            $seguimientoJuridico = SeguimientoJuridico::select($select)
                ...
                ->whereRaw('SHA1(seguimiento_juridicos.id) = "'.$key.'"')
    ...
    private function getAtencionBrindada($seguimientoJuridico) {        
        ...
        if(AtencionBrindada::whereRaw('atencion_brindada_fk is not null')->where('id', $seguimientoJuridico->atencionBrindadaSubMenu)->exists()){
            ...
            $atencion = AtencionBrindada::select(
                    ...
                    [
                        ...
                        DB::raw('SHA1(subAtencion.id) as subAtencion'), 

                        DB::raw('SHA1(atencion_brindadas.id) as atencion')
        ...
        if(AtencionBrindada::whereRaw('atencion_brindada_fk is null')->whereRaw('SHA1(id)  = "'.$seguimientoJuridico->atencionBrindada.'"')->exists()){

            $atencion = AtencionBrindada::select([DB::raw('SHA1(id) as atencion')])

            ->whereRaw('SHA1(id)  = "'.$seguimientoJuridico->atencionBrindada.'"')
    ...
    public function cambiarOpcionesCaso(Request $request){
        ...
        try{
            ...
            $caso_query = Casos::whereRaw('SHA1(id)  = "'.$request->casoKey.'"')->first();
...

    {
        ...
        try{
            ...
            $_update = SeguimientoJuridico::whereRaw('SHA1(id)  = "'.$request->key.'"')->update(['estado'=>false]);
