    ...
    <div>
        <Tabs :tabs="tabs" @selectedTab="selectedTab">
            <template v-slot:victima>
                ...
                <CodigoCasoVue :showVer="showVer ?? false" :disabledTipoCaso="formulario.key !== null"
                    :denuncias="opciones === null ? [] : opciones.denuncia" v-model:correlativo="formulario.correlativo"
                    v-model:anio="formulario.anio" v-model:mes="formulario.mes" v-model:denuncia="formulario.denuncia"
                    v-if="ocultarCodigo"></CodigoCasoVue>
                ...
                <form autocomplete="false">
                    <div class="row py-1 gy-2 mt-1 border border-2 border-primary rounded-3">
                        ...
                        <div class="col-md-6">
                            <div class="input-group border-primary flex justify-content-end">
                                <span class="input-group-text border-primary bg-primary text-white">Fecha
                                    Registro</span>
                                <input :readonly="showVer ?? false" :disabled="loading"
                                    v-model="formulario.fechaRegistro" type="datetime-local"
                                    class="form-control border-primary bg-primary text-white" id="fecha-registro"
                                    placeholder="Ingrese" />
                            </div>
                        </div>
                        <VictimaResponsableVue ref="victimaFormRef" :showVer="showVer ?? false" :loading="loading"
                            :tipo="'Víctima'" :opciones="opciones" v-model:victima="formulario.victima"
                            v-model:responsable="formulario.responsable" :vue_style_select="vue_style_select" />
                        <div class="col-md-12">
                            <div class="row col-md-3 border border-primary rounded mx-sm-1 bg-label-primary">
                                <div class="form-check m-1 text-primary">
                                    <input :readonly="showVer ?? false" :disabled="loading || (showVer ?? false)"
                                        :class="{ 'disabled': loading || (showVer ?? false) }"
                                        class="form-check-input border border-primary" type="checkbox"
                                        :checked="formulario.agresores.length > 0" id="defaultCheck-Agresores"
                                        @click="agregarQuitarAgresores" />
                                    <label class="form-check-label fw-semibold d-block" for="defaultCheck-Agresores">
                                        Añadir Agresores al Caso </label>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-12">
                    ...
                    <ButtonsFormCasosVue :showVer="showVer ?? false"
                        @handleActualizarDatosTabla="() => $emit('handleActualizarDatosTabla')" :loading="loading"
                        :showAnt="false" @resetForm="resetForm" @siguiente="siguiente('victima')"
                        @anterior="anterior('victima')" @guardar="guardar" nuevo-button>
            ...
            <template v-slot:hecho-violento>
                <small class="text-light fw-semibold">Codigo</small>
                <CodigoCasoVue :disabledTipoCaso="formulario.key !== null" :showVer="showVer ?? false"
                    :key="formulario.key" :denuncias="opciones === null ? [] : opciones.denuncia"
                    v-model:correlativo="formulario.correlativo" v-model:anio="formulario.anio"
                    v-model:mes="formulario.mes" v-model:denuncia="formulario.denuncia" v-if="ocultarCodigo">
                </CodigoCasoVue>
                <slot v-else name="codigo-caso" />
                ...
                <form>
                    <div class="row pt-2 pb-4 border border-2 border-primary rounded-3 gy-2 mt-1">
                        ...
                        <FormInputVue :titulo="'Fecha del Hecho'" :id="'caso-fecha-hecho'" :type="'date'"
                            :clases="'col-md-3'" :showVer="showVer" :loading="loading" :disabled="loading"
                            v-model:value="formulario.fechaHecho" />
                        <FormInputVue :titulo="'Hora del Hecho'" :id="'caso-hora-hecho'" :type="'time'"
                            :clases="'col-md-3'" :showVer="showVer" :loading="loading" :disabled="loading"
                            v-model:value="formulario.horaHecho" />
                        <FormSelectOpcionVue :id="'caso-circustancia-hecho'" :nombre="'Circunstancia del Hecho'"
                            :opciones="opciones === null ? [] : opciones.circunstanciaDelHecho" :clases="['col-md-6']"
                            :loading="loading" :showVer="showVer ?? false"
                            v-model:opcion="formulario.circunstanciaDelHecho"></FormSelectOpcionVue>
                        <FormDeptoMuniVue :showVer="showVer ?? false" :persona="'Ocurrencia'"
                            :departamentos="(opciones === null ? [] : opciones.departamentos)"
                            v-model:departamento="formulario.departamentoOcurrencia"
                            v-model:municipio="formulario.municipioOcurrencia"></FormDeptoMuniVue>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                ...
                                <v-select multiple placeholder="Seleccione" :style="vue_style_select"
                                    class="style-chooser" :disabled="showVer ?? false" v-if="!showVer"
                                    v-model="formulario.tiposViolencia"
                        ...
                        <FormSelectOpcionVue :id="'caso-modalidad-violencia-hecho'" :nombre="'Modalidad de Violencia'"
                            :opciones="opciones === null ? [] : opciones.modalidadViolencia" :clases="['col-md-6']"
                            :loading="loading" :showVer="showVer ?? false"
                            v-model:opcion="formulario.modalidadViolencia"></FormSelectOpcionVue>
                        <FormSelectOpcionVue :id="'caso-codigo-penal-hecho'" :nombre="'Delitos Codigo Penal'"
                            :opciones="opciones === null ? [] : opciones.delitoCodigoPenal" :clases="['col-md-6']"
                            :loading="loading" :showVer="showVer ?? false" v-model:opcion="formulario.delitoCodigoPenal"
                        ...
                        <Transition name="fade">
                            <div class="col-md-6" v-show="formulario.delitoCodigoPenal === 'Otro'">
                                <label class=" fw-semibold d-block" for="caso-codigo-penal-hecho-otro">Delitos Codigo
                                    Penal Otro (Expecifique)</label>
                                <div class="col-md">
                                    <input :readonly="showVer ?? false" v-model="formulario.delitoCodigoPenalOtro"
                                        :disabled="loading || formulario.delitoCodigoPenal !== 'Otro'" type="text"
                                        class="form-control" id="caso-codigo-penal-hecho-otro" placeholder="Ingrese" />
                                </div>
                            </div>
                        </Transition>
                        <FormSelectOpcionVue :id="'caso-institucion-que-remite'" :nombre="'Institución que Remite'"
                            :opciones="opciones === null ? [] : opciones.institucionRemitente" :clases="['col-md-6']"
                            :loading="loading" :showVer="showVer ?? false"
                        ...
                        <Transition name="fade">
                            <div class="col-md-6" v-show="formulario.institucionRemitente === 'Otra'">
                                <label class=" fw-semibold d-block" for="caso-institucion-remite-otra-hecho">Otra
                                    Institución que Remite (Expecifique)</label>
                                <div class="col-md">
                                    <input :readonly="showVer ?? false" v-model="formulario.institucionRemitenteOtra"
                                        :disabled="loading || formulario.institucionRemitente !== 'Otra'" type="text"
                                        class="form-control" id="caso-institucion-remite-otra-hecho"
                                        placeholder="Ingrese" />
                                </div>
                            </div>
                        </Transition>
                        <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                ...
                                <v-select multiple placeholder="Seleccione" :style="vue_style_select"
                                    class="style-chooser" :disabled="showVer ?? false" v-if="!showVer"
                                    v-model="formulario.tipoAsistencia"
                                    :options="opciones === null ? [] : opciones.tipoAsistencia" />
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class=" fw-semibold d-block" for="caso-institucion-remitira-hecho"> Juzgado o
                                Institución a dónde se Remite</label>
                            <div class="col-md-12">
                                <LabelShowVer :value="formulario.institucionSeRemite" v-if="showVer" />
                                <v-select multiple placeholder="Seleccione" :style="vue_style_select"
                                    :id="'caso-institucion-remitira-hecho'" :disabled="showVer ?? false" v-if="!showVer"
                                    v-model="formulario.institucionSeRemite"
                                    :options="opciones === null ? [] : opciones.institucionSeRemitira" />
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                ...
                                <v-select multiple placeholder="Seleccione" v-if="!showVer" :id="'caso-delitos-leiv'"
                                    :style="vue_style_select" :disabled="showVer ?? false"
                    ...
                    <ButtonsFormCasosVue :showVer="showVer ?? false"
                        @handleActualizarDatosTabla="() => $emit('handleActualizarDatosTabla')" :loading="loading"
                        @resetForm="resetForm" @siguiente="siguiente('hecho-violento')"
                        @anterior="anterior('hecho-violento')" @guardar="guardar">
                        <template v-slot:nuevo-button>
                            <slot name="nuevo-button" />
                        </template>
                    </ButtonsFormCasosVue>
            ...
            <template v-slot:agresores>
                <div class="py-1 px-2">
                        ...
                        <template v-for="(item, _key) in formulario.agresores" :key="_key">
                            <Transition name="fade">
                                <button class="list-group-item list-group-item-action border border-primary"
                                    ...
                                    Agresor
                                    <span class="badge badge-center bg-label-primary text-center">{{ (_key + 1)
                                        }}</span>
                                </button>
                            </Transition>
                        </template>
                        <button class="list-group-item list-group-item-action border border-primary text-primary"
                            @click="agregarAgresor" :disabled="formulario.agresores.length > 6"
                            v-if="!(showVer ?? false)">
                            Agregar <span class="badge badge-center bg-label-primary text-center"><i
                    ...
                    <div class="tab-content px-0 mt-0">
                        ...
                        <template v-for="(item, _key) in formulario.agresores" :key="_key">
                            <Transition name="fade">
                                <div class="tab-pane border border-2 border-primary rounded-3 p-2 fade"
                                    :class="{ 'show': item.selected, 'active': item.selected }"
                                    :id="'horizontal-agresor-' + (_key + 1)" v-show="item.selected">
                                    <div class="row mt-2">
                                        <div class="col-md">
                                        ...
                                        <div class="col-md" v-if="!(showVer ?? false)">
                                            <div class="float-md-end mx-3">
                                                <button type="button" class="btn btn-small btn-icon btn-danger"
                                                    @click="quitarAgresor(item)" title="Eliminar"> <i
                                    ...
                                    <div class="row gy-3 p-1">
                                        <div class="col-md-6 mb-2">
                                            <div class="form-check mt-1">
                                                <input :disabled="loading || (showVer ?? false)"
                                                    class="form-check-input"
                                                    @change="item.edad = (item.edadDesconocida ? null : item.edad)"
                                                    v-model="item.edadDesconocida" type="checkbox"
                                                    :checked="item.edadDesconocida" :id="'defaultCheck-Edad' + _key" />
                                                <label class="form-check-label fw-semibold d-block"
                                                    :for="'defaultCheck-Edad' + _key"> Edad Desconocida</label>
                                            </div>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                            <div class="form-check mt-1">
                                                <input :disabled="loading || (showVer ?? false)"
                                                    ...
                                                    v-model="item.nombreDesconocido" type="checkbox"
                                                    :checked="item.nombreDesconocido"
                                                    :id="'defaultCheck-Agresor-Nombre' + _key" />
                                                <label class="form-check-label fw-semibold d-block"
                                                    :for="'defaultCheck-Agresor-Nombre' + _key"> Nombre
                                                    Desconocido</label>
                                            </div>
                                        </div>
                                        <FormNombreApellidoVue :showVer="(showVer ?? false) || item.nombreDesconocido"
                                            :persona="'agresor-' + _key" v-model:primerNombre="item.primerNombre"
                                            v-model:segundoNombre="item.segundoNombre"
                                            v-model:primerApellido="item.primerApellido"
                                            v-model:segundoApellido="item.segundoApellido"></FormNombreApellidoVue>

                                        <FormDeptoMuniVue :key="'agresor' + _key" :showVer="showVer ?? false"
                                            :persona="'Residencia'"
                                            :departamentos="(opciones === null ? [] : opciones.departamentos)"
                                            v-model:departamento="item.departamento" v-model:municipio="item.municipio">
                                        </FormDeptoMuniVue>
                                        <FormInputVue :titulo="'Edad'" :id="'caso-edad-agresor-' + _key"
                                            :clases="'col-md-3'" :type="'number'" :min="0" :max="120" :showVer="showVer"
                                            :loading="loading" :disabled="loading || item.edadDesconocida"
                                            v-model:value="item.edad" />
                                        <FormSelectOpcionVue :id="'agresor-formacion-militar-policial-' + _key"
                                            :nombre="'Tiene Formación Militar o Policial'"
                                            :opciones="['No se Sabe', 'Si', 'No']" :clases="['col-md-6']"
                                            :loading="loading" :showVer="showVer ?? false"
                                            v-model:opcion="item.formacionMilitarPolicial"></FormSelectOpcionVue>
                                        <FormSelectOpcionVue :id="'agresor-posee-arma-' + _key" :nombre="'Posee Armas'"
                                            :opciones="['No se Sabe', 'Si', 'No']" :clases="['col-md-6']"
                                            :loading="loading" :showVer="showVer ?? false"
                                            v-model:opcion="item.poseeArma"></FormSelectOpcionVue>
                                        <FormSelectOpcionVue :id="'agresor-tipo-arma-' + _key" :nombre="'Tipo de Arma'"
                                            :opciones="opciones === null ? [] : opciones.tipoArma"
                                            :clases="['col-md-6']" :loading="loading" :showVer="showVer ?? false"
                                            v-model:opcion="item.tipoArma" @change="() => {
            item.tipoArmaOtro = (item.tipoArma !== 'Otra' || item.tipoArma === null)
                ? null : item.tipoArmaOtro;
        }"></FormSelectOpcionVue>
                                        <Transition name="fade">
                                            <FormInputVue v-show="item.tipoArma === 'Otra'"
                                                :titulo="'Otro Tipo de Arma (Especifique)'"
                                                :id="'agresor-tipo-arma-otra-' + _key" :clases="'col-md-6'"
                                                :showVer="showVer" :loading="loading"
                                                :disabled="loading || item.tipoArma !== 'Otra'"
                                                v-model:value="item.tipoArmaOtro" />
                                        </Transition>
                                        <FormSelectOpcionVue :id="'caso-relacion-agresor-victima-' + item.key"
                                            :nombre="'Relación con la Víctima'"
                                            :opciones="opciones === null ? [] : opciones.relacionVictimaAgresor"
                                            :clases="['col-md-6']" :loading="loading" :showVer="showVer"
                                        ...
                                        <Transition name="fade">
                                            <FormInputVue v-show="item.relacionVictimaAgresor === 'Otra'"
                                                :titulo="'Otra Relación Victima (Especifique)'"
                                                :id="'caso-otra-relacion-' + _key" :clases="'col-md-6'"
                                                :showVer="showVer" :loading="loading"
                                                :disabled="loading || item.relacionVictimaAgresor !== 'Otra'"
                                                v-model:value="item.otraRelacionVictima" />
                                        </Transition>
                                        <FormSelectOpcionVue :id="'caso-ocupacion-agresor-' + _key"
                                        ...
                                        <Transition name="fade">
                                            <FormInputVue v-show="item.ocupacion === 'Otro'"
                                                :titulo="'Otra Ocupación Agresor (Especifique)'"
                                                :id="'caso-ocupacion-agresor-otro-' + _key" :clases="'col-md-6'"
                                                :showVer="showVer" :loading="loading"
                                                :disabled="loading || item.ocupacion !== 'Otro'"
                                                v-model:value="item.ocupacionOtro" />
                                        </Transition>
                                        <FormInputVue :titulo="'Dirección / Domicilio'"
                                            :id="'caso-direccion-agresor-' + _key" :clases="'col-md-4'"
                                            :showVer="showVer" :loading="loading" v-model:value="item.direccion" />
                                        <FormSelectOpcionVue :id="'agresor-zona-residencial-agresor-' + _key"
                                            ...
                                            v-model:opcion="item.zonaResidencial"></FormSelectOpcionVue>
                                        <FormInputVue :titulo="'Lugar de Trabajo'" :id="'caso-lugar-trabajo-' + _key"
                                            :clases="'col-md-6'" :showVer="showVer" :loading="loading"
                                            v-model:value="item.lugarTrabajo" />
                                        <FormInputVue :titulo="'Dirección de Trabajo'"
                                            :id="'caso-direccion-trabajo-' + _key" :clases="'col-md-6'"
                                            :showVer="showVer" :loading="loading"
                    ...
                    <ButtonsFormCasosVue :showVer="showVer ?? false"
                        @handleActualizarDatosTabla="() => $emit('handleActualizarDatosTabla')" :loading="loading"
                        @resetForm="resetForm" @siguiente="siguiente('agresores')" @anterior="anterior('agresores')"
                        @guardar="guardar">
                        <template v-slot:nuevo-button>
                            <slot name="nuevo-button" />
                        </template>
                    ...
                    <div class="row  py-2 border border-2 border-primary rounded-3">
                        ...
                        <div class="col-md-12" v-show="!(showVer ?? false)">
                            <form ref="formArchivosRef">
                                <label v-if="!(showVer ?? false)" for="formFileMultipleCaso"
                                    class="fw-semibold d-block">Seleccionar Archivos</label>
                                <input v-if="!(showVer ?? false)" :readonly="showVer ?? false"
                                    accept="application/msexcel, application/msword, application/pdf, application/rtf, image/*"
                                    class="form-control btn-outline-primary" type="file"
                                    v-on:change="handleSubidaArchivos" ref="archivosCasos" id="formFileMultipleCaso"
                                    multiple :disabled="loading" />
                            </form>
                        </div>
                        <div class="col-md-6 p-2 mt-3" v-if="!(showVer ?? false)">
                            ...
                            <ul class="list-group-archivos list-group border border-1 border-primary" v-auto-animate>
                                ...
                                <h5 class="text-wrap text-nowrap mt-2 text-center"
                                    v-show="archivosCasoArray.length === 0">
                                    <strong><i class="bx bx-md bx-no-entry text-danger"></i> No existen
                                        archivos.</strong>
                                </h5>
                            </ul>
                        </div>
                        <div class="p-2 mt-3" :class="!(showVer ?? false) ? 'col-md-6' : 'col-md-12'">
                            ...
                            <ul class="list-group-archivos list-group border border-1 border-primary" v-auto-animate>
                                        ...
                                        <Popper class="popper-theme text-start" arrow v-if="!(showVer ?? false)">
                                            ...
                                            <template #content="{ close }">
                                                <div class="dropdown-header">
                                                    <h6 class="text-wrap text-nowrap "><strong>Esta acción no se podra
                                                            revertir,<br> ¿Continuar?</strong></h6>
                                                </div>
                                                <button class="dropdown-item text-start-center"
                                                    @click="handledBorrarArchivo(item.key); close();"
                                                    :disabled="loading">
                                                    <i class="bx bx-check"></i> Continuar
                                                </button>
                                                <button class="dropdown-item text-start-center" @click="close();"
                                                    :disabled="loading">
                                ...
                                <h5 class="text-wrap text-nowrap mt-2 text-center"
                                    v-show="formulario.archivosCasos.length === 0">
                                    <strong><i class="bx bx-md bx-no-entry text-danger"></i> No existen
                                        archivos.</strong>
                                </h5>
                            </ul>
                        ...
                        <div>
                            ...
                            <textarea type="text" class="form-control" v-model="formulario.observaciones">{{ formulario.observaciones
        }}</textarea>
                        </div>
                    </div>
                    <ButtonsFormCasosVue :showVer="showVer ?? false"
                        @handleActualizarDatosTabla="() => $emit('handleActualizarDatosTabla')" :loading="loading"
                        :showSig="false" @resetForm="resetForm" @siguiente="siguiente('agresores')"
                        @anterior="anterior('agresores')" @guardar="guardar">
                        <template v-slot:nuevo-button>
                            <slot name="nuevo-button" />
                        </template>
                    </ButtonsFormCasosVue>
